...
- [x] 学生UI：課題文表示 → タップ録音 → 単語別色分け結果 → 弱点語リスト自動更新
- [x] AI会話練習：既存の介護シナリオを流用し、自己紹介 / 職場会話 / 報連相 の3シナリオを追加
- [x] streak（連続利用日数）表示
...
## Phase 2：面接ロールプレイ（2日）
- [x] 面接官ペルソナprompt 3種（優先順：介護 → 食品製造 → 外食）。企業設定、質問フロー（自己紹介 → 志望理由 → 想定質問 → 逆質問）、5〜8ターン
- [x] テキストモードを先行実装（プロンプトとフローの検証用）
- [x] 音声モード：録音 → STT(ja-JP) → Bedrock Claude → Neural TTS再生のターン制カスケード（既存音声ループを流用）
- [x] ルーブリック評価：軸 = 日本語力 / 内容の一貫性 / 敬語・礼儀 / 報連相 / 発話明瞭さ（発音スコア連携）。構造化JSON出力、rubric_version保存、総合0〜100
- [x] フィードバック画面：インドネシア語の解説 + 日本語の模範解答例
- [x] 面接履歴一覧とスコア推移グラフ（`/interview/history` + 学生UIの推移チャート・過去フィードバック再表示）
...
- [x] オリジナル問題バンク100問をseed（文法40 / 語彙40 / 読解20。N4 / JFT-Basic水準タグ。過去問の複製は禁止）。模試の聴解用に12問を追加（計112問）
- [x] 問題生成スクリプト：Claudeで問題案を生成 → review_flag付きで保存（人間の確認前は出題しない）。`scripts/quiz/generate_quiz_drafts.py`（generate / list / approve）
- [x] デイリークイズ10問 + 誤答再出題（簡易SRS）
- [x] 模試モード25問、スコア換算。listening問題はTTSで音源生成
- [x] 画面に「非公式・試験対策用」表記
...
- [x] 集計サービス：passport snapshot(jsonb) = 日本語レベル推移（N5→N4）、発音平均、会話スコア、面接準備度（直近評価 + 文字起こし抜粋）、職種別チェックリスト、出席率、生活態度、リスクフラグ
- [x] リスクフラグはルールベース：出席率80%未満 / スコア下降 / 7日間未利用
- [x] 教師入力UI：出席（月次% or 日次）、態度チェックリスト（報連相・時間厳守・寮生活・マナー等5項目）。学生詳細ページ + Passport生成ボタン込み
- [x] Passport画面（日本語）→ WeasyPrintでA4 PDF（1〜2枚、Noto Sans JP同梱、企業提出用の候補者紹介シート体裁）
- [x] 共有リンク：ランダム32byteトークン、有効期限30日、失効操作、閲覧ログ
...
- [x] 教師：クラス一覧（進捗 / 最終利用 / アラート）、学生詳細（スコア推移・弱点語・面接履歴・文字起こし）、添削キュー
- [x] 経営者KPI：N4到達率、模試平均、出席率、練習量、週次トレンド、PoC KPIカード
- [x] 企業ビュー：候補者比較テーブル（ソート可）→ 各Passportへ。トークンアクセス（ログイン不要）
- [x] リスクフラグ学生のアラートバッジ（教師クラス一覧・学生詳細・経営者KPIに表示。企業ビューには出さない方針）
...
- [x] docs/DEMO.md：10分デモ台本 + フォールバック（事前録画）メモ + スモークチェックリスト
